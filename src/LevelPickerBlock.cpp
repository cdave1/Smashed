...
void LevelPickerBlock::UpdateInterface()
{
    ...
    if (m_levelStats->highestCompletionPercentage >= 100 * HDK_GAME_GOAL_MIN_PROPORTION)
    {
        m_imgBronzeStar->Show();
    }
    if (m_levelStats->highestCompletionPercentage >= 100 * HDK_GAME_GOAL_MIDDLE_PROPORTION)
    {
        m_imgBronzeStar->Hide();
        m_imgSilverStar->Show();
    }
    if (m_levelStats->highestCompletionPercentage >= 100 * HDK_GAME_GOAL_MAX_PROPORTION)
    {
        m_imgBronzeStar->Hide();
        m_imgSilverStar->Hide();
        m_imgGoldStar->Show();
